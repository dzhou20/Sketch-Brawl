...
## 3. Run Backend + Battle Worker
```bash
cd backend
poetry run alembic upgrade head
poetry run uvicorn src.api.main:app --reload --port 8000
poetry run celery -A src.workers.battle_worker worker -l info
```

## 4. Run Telemetry Exporter
```bash
cd backend
...
- **Deterministic Battles**: execute `poetry run pytest tests/integration/test_battle_api.py` and open `http://localhost:8000/battles/{id}/events` to confirm the SSE stream matches stored timelines.
...
```
Publish new artifact metadata via `/artifacts/latest` endpoint (should return playable/video/offline URLs + version).
